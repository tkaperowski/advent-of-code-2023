...
# for is not working as range() is not updated keeping starting value
i=0
while i < len(lines):
    print(len(lines))
    ...
    # add cards when count > 0
    while count > 0:

        # get winning card number
        card = re.findall('(\d+):', lines[i])
        card_number = int(card[0])

        # find card number to add, which is card_number + count
        for n in range(i+1, len(lines)):
            next_card = re.findall('(\d+):', lines[n])
            next_card_number = int(next_card[0])
            # add new card, starting from high card numbers
            if next_card_number == card_number + count:
                lines.insert(n+1, lines[n]) # add element to lines, extends its length thus while not for should be used in line 45
                break
        # reduce count to add another card with lower number
        count -= 1
    # check next card
    i += 1
end = time.time()
print(f'running time: {end-start} [s]')
print(f'all cards: {len(lines)}')