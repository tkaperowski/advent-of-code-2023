...
for input in history:
    ...
    while i > 0:
        ...
        if all(x == 0 for x in my_dict[f'diff_seq_{i}']):
            print('found all-zeros list')
            i -= 1
            # add element to list above all-zeros list
            my_dict[f'diff_seq_{i}'].append(my_dict[f'diff_seq_{i}'][-1])
    ...
    while i > -1:
        if i == 0:
            ...
            prediction = my_dict[f'diff_seq_{i}'][-1] + my_dict[f'diff_seq_{i + 1}'][-1]
            break
        # add element which is sum of last elements of this list and list below
        my_dict[f'diff_seq_{i}'].append(my_dict[f'diff_seq_{i}'][-1] + my_dict[f'diff_seq_{i + 1}'][-1])
        # print(my_dict[f'diff_seq_{i}'])
        i -= 1
